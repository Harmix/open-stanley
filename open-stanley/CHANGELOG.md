...
Three changes, all from questions the user asked after two weeks of real runs.

**Official API before the browser.** The `publish` skill said the browser was the default and gave as its reason that "LinkedIn has no public posting API for individuals", which is wrong: `w_member_social` is self-serve and posts, comments and likes as the authenticated member. X shipped a first-party hosted MCP at `api.x.com/mcp` on 30 June 2026 whose free tier covers writes. New reference `skills/stanley/references/platform-access.md` splits every platform into writes (official MCP/API first) and reads (browser, because neither platform sells an individual a self-serve read surface), gives the procedure for a platform the plugin has never seen, and carries the browser-pacing rules for when the browser is the only path. `my-human.md` records `post_path:` per platform. Context: a browser-posted X thread on 12 Sep was followed by a suspension on 13 Sep — unproven as cause, and the reason the order is now what it is.

**A weekly improvement rung.** The monthly strategy check-in was the only ritual whose output was a change to how the plugin works, and monthly is the wrong frequency for the signals that actually accumulate: draft→final diffs, skips, and audit overrides all arrive weekly, while engagement metrics need a month to mean anything. New `tune` ritual (Friday 17:00) and scheduled-task template, running `learn` on the week's edits and reporting the active experiment's arms without calling a winner before both arms hold four posts. `scheduling.md` gains the cadence ladder (daily reacts, weekly tunes, monthly re-strategises, quarterly re-scopes) and the Fri→Sat→Sun ordering: rules change, then material is mined, then drafts are written. The monthly keeps what only it can do — close or rotate the experiment, study one outside creator so the loop imports moves instead of recombining old ones, and check whether a flagship has run this quarter.

**Longform.** New `longform` skill: one teardown, field report or build log a month, written from an evidence ledger where every line carries a receipt, then cut into 4–6 posts, 2–3 comment angles and a soapbox question. Each shape has an entry condition checked against `stories/` before a word is written, so a thin piece is refused rather than padded. `stanley-audit --platform longform` adds word-band, receipt-density, listicle and summary-box checks. Prior art credited: the teardown skill at `github.com/MisreadableMind/human-writing-skill`, shared by Vitalii Ratushnyi after the team read a memory teardown they wished they had written.
